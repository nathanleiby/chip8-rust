...
# FROM: https://gist.github.com/tgolsson/d78f7887a8542f3fd6f125070e5e22d6?permalink_comment_id=4901010#gistcomment-4901010
#
set -e
...
# Using `gsed` instead of MacOS default `sed` .. gsed is the linux version from Homebrew
gsed -i "s/import \* as __wbg_star0 from 'env';//" dist/"$PROJECT_NAME".js
gsed -i "s/let wasm;/let wasm; export const set_wasm = (w) => wasm = w;/" dist/"$PROJECT_NAME".js
gsed -i "s/imports\['env'\] = __wbg_star0;/return imports.wbg\;/" dist/"$PROJECT_NAME".js
gsed -i "s/const imports = __wbg_get_imports();/return __wbg_get_imports();/" dist/"$PROJECT_NAME".js
